...
namespace eval ::TE {
  namespace eval INIT {
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------------
    #--------------------------------
    #--run_board_selection: prototype to select board interactive 
    proc run_board_selection {} {
      TE::UTILS::te_msg TE_INIT-89 INFO "Run TE::INIT::run_board_selection: Prototype"
      ...
      while {! [string match -nocase "Q" $someVar ] } {
        if {[string match -nocase "L" $someVar ] } {
          ...
          set tmpList $tmpList2
          set someVar Q
        } elseif {[string match -nocase "ID" $someVar ] } {
          ...
          puts "Go back to top menu with 'q' or 'Q'"
          puts "Insert FPGA/SoC Part Name:"
          gets stdin someVar
          if {![string match -nocase "Q" $someVar ] } {
          ...
          puts "Go back to top menu with 'q' or 'Q'"
          puts "Insert PCB Revision Number:"
          gets stdin someVar
          if {![string match -nocase "Q" $someVar ]} {
        ...
        if {[llength $tmpList] == 2 } {
          puts "Should the project create with this device? y/N"
          gets stdin someVar
        ...
        if {[string match -nocase "Q" $someVar ]} {
          ...
          puts "-Reduce List by ID, press: 'ID' or 'id' "
      ...
      if { [string match -nocase "Y" $someVar ]} {
        ...
        TE::UTILS::mod_configfile [lindex $selected ${TE::BDEF::ID}]
    ...
    proc run_project {BOARD RUN GUI CLEAN} {
      ...
      switch $CLEAN {
        ...
        2 {
            ...
            if {[catch {TE::UTILS::clean_workspace_hsi} result]} {abort_status "Error Initialization..."; create_allboardfiles_status; TE::UTILS::te_msg TE_INIT-134 ERROR "Script (TE::UTILS::clean_workspace_hsi) failed: $result."; return -code error}
          }
        3 {
    ...
    proc generate_board_file_project_all {GUI SDSOC} {
      ...
      foreach sublist $TE::BDEF::BOARD_DEFINITION {
        ...
        if {$id ne "ID" } {
          ...
          if {$rundesign == true} {
            ...
            if {[catch {TE::UTILS::clean_workspace_hsi} result]} {abort_status "Error Initialization..."; create_allboardfiles_status; TE::UTILS::te_msg TE_INIT-150 ERROR "Script (TE::UTILS::clean_workspace_hsi) failed: $result."; return -code error}
      ...
      for {set i 0} {$i < [llength $TE::TE_INTERNAL_EXPORT]} {incr i} {
        set export_typ [lindex $TE::TE_INTERNAL_EXPORT $i]
        set export_app [lindex $TE::TE_INTERNAL_APP $i]
        if {[catch {TE::ADV::beta_hw_export_binary -mode $export_typ -app $export_app -all} result]} {abort_status "Error Initialization..."; create_allboardfiles_status; TE::UTILS::te_msg TE_INIT-171 ERROR "Script (TE::ADV::beta_hw_export_binary) failed: $result."; return -code error}
      }
