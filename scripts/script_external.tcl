...
# ------------------------------------------
# -- $Date:  2019/04/8  | $Author: Hartfiel, John
# -- - add run_putty
# ------------------------------------------
...
namespace eval ::TE {
  namespace eval EXT {
    ...
    proc generate_app_bit_mcs {{fname ""}} {
      ...
      set checkfile [TE::UTILS::prebuilt_file_location false ${TE::VPROJ_NAME} .mmi NA NA NA false]
      if {! [string match "NA" $checkfile] } { 
        ...
        foreach sw_applist_line ${TE::SDEF::SW_APPLIST} {
          ...
          if {[lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "0"} {
            ...
            #
            set mmi_abs [TE::UTILS::prebuilt_file_location false ${TE::VPROJ_NAME} .mmi NA NA NA false]
            set elf_abs [TE::UTILS::prebuilt_file_location false ${app_name} .elf NA NA NA false]
            set bit_abs [TE::UTILS::prebuilt_file_location false ${TE::VPROJ_NAME} .bit NA NA NA false]
          ...
          if {[lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "0" || [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "FIRM"} {
           if {$TE::CFGMEM_MEMSIZE_MB ne "NA"} {
              ...
              if {[lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "FIRM"} {
                set temp_rel [TE::UTILS::prebuilt_file_location true ${TE::VPROJ_NAME} .bit NA NA ${curr_dir} false]
                set load_data "up 0x0 ${temp_rel} "
              } else {
                set temp_rel
                set load_data "up 0x0 ./${app_name}.bit "
              }
              #get upload data 01:
        ...
        TE::UTILS::te_msg TE_SW-10 WARNING "${TE::PREBUILT_HW_PATH}/${int_shortdir}/${TE::VPROJ_NAME}.mmi doesn't exist. Nothing is done."
      }
    }
    ...
    proc generate_bif_files {{fname ""}} {
      ...
      set checkfile [TE::UTILS::prebuilt_file_location false ${TE::VPROJ_NAME} .bit NA NA NA false]
      if {[string match "NA" $checkfile] } { 
        TE::UTILS::te_msg TE_SW-11 ERROR "Bit-file (${TE::VPROJ_NAME}.bit)  doesn't exist in the prebuilt folders."
        return -code error "Bit-file (${TE::VPROJ_NAME}.bit) doesn't exist in the prebuilt folders.";
      ...
      foreach sw_applist_line ${TE::SDEF::SW_APPLIST} {
        ...
        if {[lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "0" || [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "1" || [lindex $sw_applist_line ${TE::SDEF::STEPS}] eq "FSBL_APP"} {
          ...
          if {$TE::IS_ZSYS} {
            ...
            set destination_cpu [lindex $sw_applist_line ${TE::SDEF::ZYNQMP_DESTINATION_CPU}]
        ...
        if {$use_basefolder} {
          ...
          if { ![catch {set binfilename [glob -join -dir ${TE::BASEFOLDER}/ *.bin]}] } {
            ...
            if {[file exists ${TE::BASEFOLDER}/zynqmp_fsbl_flash.elf]} {
              set fsblfile ${TE::BASEFOLDER}/zynqmp_fsbl_flash.elf   
            } elseif {[file exists ${TE::BASEFOLDER}/zynq_fsbl_flash.elf]} {
              set fsblfile  ${TE::BASEFOLDER}/zynq_fsbl_flash.elf            
            } else {
              TE::UTILS::te_msg TE_PR-89 {CRITICAL WARNING} "Zynq FSBL file (${TE::BASEFOLDER}/zynq_fsbl_flash.elf or zynqmp_fsbl_flash.elf) doesn't exist." 
            }  
          } else {
          ...
          set bootbinname BOOT.bin
          if {[file exists ${TE::PREBUILT_SW_PATH}/${int_shortdir}/zynqmp_fsbl_flash.elf]} {
            set fsblfile ${TE::PREBUILT_SW_PATH}/${int_shortdir}/zynqmp_fsbl_flash.elf   
          } elseif {[file exists ${TE::PREBUILT_SW_PATH}/${int_shortdir}/zynq_fsbl_flash.elf]} {
            set fsblfile  ${TE::PREBUILT_SW_PATH}/${int_shortdir}/zynq_fsbl_flash.elf            
          } else {
            TE::UTILS::te_msg TE_PR-90 {CRITICAL WARNING} "Zynq FSBL file (${TE::PREBUILT_SW_PATH}/${int_shortdir}/zynq_fsbl_flash.elf or zynqmp_fsbl_flash.elf) doesn't exist." 
          } 
    ...
    proc svn_checkin {foldername {mgs ""}} {
      ...
      ------"
      set result [eval $command]
      TE::UTILS::te_msg TE_EXT-6 INFO "Command results from SVN check in \"$command\": \n \
        $result \n \
      ------"  
      cd $cur_path 
      }
    ...
    proc zip_project {zipname {excludelist ""}} {
      ...
      if {${TE::ZIP_PATH} ne ""} {
        ...
        ------"
        set result [eval $command]
        TE::UTILS::te_msg TE_EXT-8 INFO "Command results from ZIP \"$command\": \n \
          $result \n \
        ------"  
        } else {
