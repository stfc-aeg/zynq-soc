...
namespace eval ::TE {
  namespace eval SDSOC {
    ...
    #clear old sdsoc
      ...
      set hello_world [glob -directory "${TE::WORKSPACE_HSI_PATH}" hello_*]
	  ...
	  TE::UTILS::te_msg TE_SDSOC-5 ERROR "Hello_world application NOT found, not adding bsp headers to platform"
      }
	  ...
	  # create "include" folder before writing its content
	  ...
	  TE::UTILS::te_msg TE_SDSOC-6 ERROR "Linux platform will be generated without BSP headers"
      }
      ...
      if {$TE::IS_ZUSYS} { #zynqMP
        ...
        if {[file exists ${linker_script}]} {
         ...
         TE::UTILS::te_msg TE_SDSOC-7  {CRITICAL WARNING} "Predefined Linker Scripted not found on ${TE::SET_PATH}/sdsoc/generic/sw/a53_standalone/."
        }
        # --------------------------------------------
        # prebuilt files
        # set search paths
    ...
    proc create_sdsoc_pfm {} {
      ...
      cd ${TE::TMP_PATH}
      exec xsct -sdx ${TE::SET_PATH}/sdsoc/generic/${ps_type}_sdsoc_simple_pfm.tcl
      ...
      cd ${TE::TMP_PATH}
      exec xsct -sdx ${TE::SET_PATH}/sdsoc/generic/${ps_type}_sdsoc_pfm.tcl
